...
muestras <- rbinom(n = 20000, size = n_pruebas, prob = p_exito)

# b) Probabilidad de al menos 45 éxitos (P(X >= 45), 6 sumas )
prob_simulada <- mean(muestras >= 45)               # muestras>=45 es una condicion, lo que convierte al vector muestras en un vector de 1's y 0's
...
# Comparación
cat("Probabilidad simulada (≥45 éxitos):", prob_simulada, "\n")
cat("Probabilidad teórica (≥45 éxitos):" , prob_teorica,  "\n")

# c) Histograma y densidad teórica
hist(
   muestras,                                        #vector de valores (en este caso, pruebas exitosas)
